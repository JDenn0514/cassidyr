...
- When working on cassidyr, gather context with `cassidy_context_project(level = "comprehensive")`
...
- **Configuration files** - Both CASSIDY.md (for the package) and CLAUDE.md (for development) use modular structure
...
5. **Explain design decisions** - help understand the "why" behind choices
...
- **CASSIDY.md + .cassidy/rules/** - For the package's own context system (used by `cassidy_app()`)
- **CLAUDE.md + .claude/rules/** - For Claude Code development (what you're reading now)

Both use identical modular structures to demonstrate the pattern the package implements.
