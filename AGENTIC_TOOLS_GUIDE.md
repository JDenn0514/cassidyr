...
The cassidyr agentic system can **automatically choose which tools to use** based on the task. By default, all 7 tools are available, and the AI assistant intelligently selects the appropriate ones.
...
Use `cassidy_tool_preset()` for common patterns:
...
**Safe mode is ON by default** and requires approval for risky operations:
...
- **n** = Deny (agent will try different approach)
- **e** = Edit parameters before executing
- **v** = View detailed tool information
...
Agent will:
1. Use `list_files` to check if R/utils.R exists
2. Use `read_file` to see existing code
3. Request approval to use `write_file`
...
Agent will:
1. Use `describe_data` to understand mtcars structure
2. Use `execute_code` to compute correlations
...
Agent will:
1. Use `list_files` to find test files
2. Use `search_files` to find agentic-related tests
3. Use `read_file` to read test files
4. Use `get_context` for project structure
...
**Default behavior:** All tools available, safe mode ON, max 10 iterations.
...
- `?cassidy_agentic_task` - Main function documentation
- `?cassidy_list_tools` - View available tools
- `?cassidy_tool_preset` - Tool preset documentation
