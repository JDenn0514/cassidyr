...
1. **User-level memory**: `~/.cassidy/CASSIDY.md` and `~/.cassidy/rules/*.md`
...
This design keeps memory files discoverable and predictable - anyone who clones your R package will see exactly what context is being used.

### Optional: Recursive Search

If you're working in a large monorepo or nested project structure, you can enable upward recursive search:
...
Create personal preferences that apply to all your projects in `~/.cassidy/CASSIDY.md`:
...
Remember: *cassidyr* searches *up* the directory tree from your working directory, not down. If you have `project/subdir/CASSIDY.md` and you are working from `project/`, that file will not be loaded. It is only loaded when you work from `project/subdir/` or deeper.
